...
# file = os.path.join("tech-tags.txt")
# file = open(file, "r", encoding='utf-8')    
...
#     )
    ...
    def find_segment_indices(string_to_search, resume_segments, resume_indices):
        for i, line in enumerate(string_to_search):
            ...
            if [a for a in resumeparse.accomplishments if header.startswith(a)]:
                try:
            ...
            elif [e for e in resumeparse.certification_and_training if header.startswith(e)]:
                try:
                    resume_segments['certification_and_training'][header]
                except:
                    resume_indices.append(i)
                    header = [e for e in resumeparse.certification_and_training if header.startswith(e)][0]
                    resume_segments['certification_and_training'][header] = i
            ...
            elif [a for a in resumeparse.projects if header.startswith(a)]:
                try:
                    resume_segments['projects'][header]
                except:
                    resume_indices.append(i)
                    header = [a for a in resumeparse.projects if header.startswith(a)][0]
    ...
    def extract_li(text):
        p=re.findall(r'(http:|https:)(/|//)(www\.)(linkedin\.)(com/)(in?)+(/)([a-zA-Z0-9-]+)',text)
        if p:
            p=list(p[0])
            linekdin="".join(p)
            return linekdin
    ...
    def read_file(file):
        ...
        li=resumeparse.extract_li(full_text)
        ...
        #################Extracting Trainings and certifications##############
        training=""
        if len(resume_segments['certification_and_training'].keys()):
            for key , values in resume_segments['certification_and_training'].items():
                training += re.sub(key, '', ",".join(values), flags=re.IGNORECASE)            
            training = training.strip().strip(",").split(",")
        
        
        #################Extracting projects################################
        accomp=""
        if len(resume_segments['projects'].keys()):
            for key , values in resume_segments['projects'].items():
                accomp += re.sub(key, '', ",".join(values), flags=re.IGNORECASE)            
            accomp = accomp.strip().strip(",").split(",")
            
        #####################Extracting accomplishments#####################
        a=""
        if len(resume_segments['accomplishments'].keys()):
            for key , values in resume_segments['accomplishments'].items():
                a += re.sub(key, '', ",".join(values), flags=re.IGNORECASE)            
            a = a.strip().strip(",").split(",")
            
            #################Extracting skills##############
        
        skills = []
        file = os.path.join(base_path,"mytech.txt")
        file = open(file, "r", encoding='utf-8')    
        skill = [line.strip().lower() for line in file]
        nlp = English()
        ruler = EntityRuler(nlp)
        skillsmatcher = PhraseMatcher(nlp.vocab, attr="LOWER")
        patterns = [nlp.make_doc(text) for text in skill]
